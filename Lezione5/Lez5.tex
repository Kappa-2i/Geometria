...
\Definizione{Combinazione Lineare}{
    Consideriamo \((V,+,\cdot)\) uno spazio vettoriale su \(K\). Sia \((u_1,...,u_n)\) una n-upla di vettori di \(V\). Un vettore \(u\) si dice 
    \textbf{combinazione lineare} di questa n-upla se esiste una n-upla di scalari \((\alpha_1,...,\alpha_n) \in K\) tali che 
    \[\mathbf{u} = \alpha_1 u_1 + ... + \alpha_n u_n\].
...
\Definizione{Sistema di Generatori}{
    ...
    Un insieme \(S \subseteq V\) si dice \textbf{sistema di generatori} di \(V\) se ogni vettore di \(V\) può essere espresso come combinazione lineare di un numero finito di vettori di \(S\).  
    In simboli:
    \[
        V = L(S) = \left\{ \sum_{i=1}^{n} \lambda_i v_i \ \middle| \ v_i \in S, \ \lambda_i \in K, \ n \in \mathbb{N} \right\}.
    \]
    In altre parole, i vettori di \(S\) \emph{generano} lo spazio \(V\).  
    Se esiste un insieme finito \(S\) che genera \(V\), allora si dice che \(V\) è \textbf{finitamente generato}.
...
\Definizione{Linearmente dipendente e indipendente}{
    Una \(n-pla (v_1,...,v_n)\) di vettori di \(V\) spazio vettoriale su \(K\) sarà detta \textbf{linearmente dipendente} se esiste
    una \(n-pla (\alpha_1,...,\alpha_n) \in K\) di scalari non tutti nulli, tale che 
