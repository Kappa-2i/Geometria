...
\Definizione{Kernel (o nucleo)}{
    Data un'applicazione lineare \( T : V \to W \), si definisce \textbf{Kernel} (o \textbf{nucleo}) di \( T \) l'insieme:
    \[
...
\newpage
\section{Teorema dell'Equazione Dimensionale}
...
\BoxBlue{Osservazione}{Gli isomorfismi conservano la L.I.}{
    Sia \(T: V \to W\) un \textbf{isomorfismo}. 
    Sia \(V\) uno spazio vettoriale su campo \(K\) (ad esempio, con \(\dim V = n\)) e sia \(S \subseteq V\) un sottoinsieme di vettori.
    
    Allora \(S\) è \textbf{linearmente indipendente} \(\iff\) l'insieme delle immagini \(T(S)\) è \textbf{linearmente indipendente}.
    
    \textit{(Questo è una conseguenza diretta del fatto che, essendo \(T\) un isomorfismo, \(T\) è iniettiva (\(\ker(T) = \{\bar{0}_V\}\), che dimostra \((\Rightarrow)\)), e anche la sua inversa \(T^{-1}: W \to V\) è un isomorfismo, che dimostra \((\Leftarrow)\)).}
}
